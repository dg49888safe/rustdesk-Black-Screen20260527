...
class MainActivity : FlutterActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Check and prompt for battery optimization whitelist (for MIUI/HyperOS and other aggressive ROMs)
        checkAndPromptBatteryOptimization()
    }

    private fun checkAndPromptBatteryOptimization() {
        val prefs = getSharedPreferences(KEY_SHARED_PREFERENCES, MODE_PRIVATE)
        val alreadyRequested = prefs.getBoolean(KEY_BATTERY_OPT_REQUESTED, false)
        if (!alreadyRequested && !isIgnoringBatteryOptimizations(this)) {
            ...
            when (call.method) {
                ...
                GET_VALUE -> {
                    if (call.arguments is String) {
                        ...
                        } else if (call.arguments == "battery_optimization_ignored") {
                ...
                "request_battery_whitelist" -> {
                    requestIgnoreBatteryOptimizations(context)
                    result.success(true)
                }
                "schedule_keepalive_job" -> {
                    KeepAliveJobService.scheduleKeepAliveJob(context)
                    result.success(true)
                }
                "cancel_keepalive_job" -> {
                    KeepAliveJobService.cancelKeepAliveJob(context)
                    result.success(true)
                }
                "is_keepalive_job_scheduled" -> {
                    result.success(KeepAliveJobService.isJobScheduled(context))
                }
