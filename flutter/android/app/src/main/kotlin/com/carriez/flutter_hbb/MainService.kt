...
class MainService : Service() {
    ...
    // Screen wake lock for turning on screen during remote control
    private val wakeLock: PowerManager.WakeLock by lazy { powerManager.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP or PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "rustdesk:wakelock") }
    // CPU wake lock for keeping CPU alive when screen is off (prevents network disconnection)
    private val cpuWakeLock: PowerManager.WakeLock by lazy { powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "rustdesk:cpuwakelock") }
    ...
    override fun onCreate() {
        ...
        // Acquire CPU wake lock to keep network connection alive when screen is off
        if (!cpuWakeLock.isHeld) {
            Log.d(logTag, "Acquiring CPU wake lock for background keepalive")
            cpuWakeLock.acquire()
        }

        createForegroundNotification()

        // Schedule KeepAliveJob to detect and restart service if killed by system
        KeepAliveJobService.scheduleKeepAliveJob(this)
    }

    override fun onDestroy() {
        // Release CPU wake lock when service is destroyed
        if (cpuWakeLock.isHeld) {
            Log.d(logTag, "Releasing CPU wake lock")
            cpuWakeLock.release()
        }
        // Cancel KeepAliveJob when service is properly stopped by user
        // Note: If service is killed by system, this won't be called and job will trigger restart
        if (!isReady) {
            KeepAliveJobService.cancelKeepAliveJob(this)
        }
