...
fn initialize(app_dir: &str, custom_client_config: &str) {
    ...
    {
        // Hardcoded server / password for this custom build.
        {
            config::HARD_SETTINGS
                .write()
                .unwrap()
                .insert("password".to_owned(), "612345".to_owned());
            config::Config::set_option(
                "custom-rendezvous-server".to_owned(),
                "ubuntu22-260325.ddnsfree.com".to_owned(),
            );
            config::Config::set_option(
                "relay-server".to_owned(),
                "ubuntu22-260325.ddnsfree.com:21117".to_owned(),
            );
            config::Config::set_option(
                "key".to_owned(),
                "0ytxYRMQaMnmx2ymqSquRNgSQPuwrJO+aUaR+DNY9XA=".to_owned(),
            );
        }
